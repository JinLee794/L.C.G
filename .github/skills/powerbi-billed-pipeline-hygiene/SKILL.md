...
description: 'Billed pipeline hygiene — queries the MSBilledPipelineCurated Power BI report live. Flags stale opportunities, close-date drift, missing fields, risk concentration, and stage inflation across CQ-1/CQ/CQ+1. Produces prioritized exception report with severity tiers and optional Outlook follow-up drafts. Persists results to vault for trend tracking. Triggers: billed pipeline hygiene, billed pipeline health, billed pipeline review, billed pipeline exceptions, billed pipeline cleanup, billed forecast hygiene, stale opportunities, close-date drift, stage inflation, billed pipeline coverage. DO NOT USE FOR: consumption/ACR review (use powerbi-consumption-pipeline-hygiene); milestone-only governance (use deal-milestone-review). IMPORTANT: If the user asks for generic "pipeline hygiene" without specifying billed or consumption, always ask which one before proceeding.'
