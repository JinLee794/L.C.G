...
## Pre-Flight: Query Scoping

**Before running any operation**, check that the request has explicit time bounds AND at least one entity constraint (person, customer, or meeting name).

If either is missing → apply `internal-workiq-scoping` first to build the fact map and resolve entities, then return here to execute the matched operation with the scoped inputs.

| Has explicit time + entity? | Action |
|---|---|
| Yes | Proceed directly to the matching operation below |
| Time missing OR entity missing | Run `internal-workiq-scoping`, then continue |
| Both missing ("everything", "all my notes") | Run `internal-workiq-scoping` — do not proceed without a bounded fact map |
